...
function buildPath(points: TrendPoint[], minValue: number, maxValue: number) {
  ...
  return points
    .map((point, index) => {
      ...
      const y =
        top + ((maxValue - point.weightKg) / valueRange) * chartHeight;
...
export function WeightTrendChart({ points }: { points: TrendPoint[] }) {
  ...
  const values = filteredPoints.map((point) => point.weightKg);
  const minValue =
    values.length > 0 ? Math.max(0, Math.floor(Math.min(...values) - 2)) : 0;
  const maxValue =
    values.length > 0 ? Math.ceil(Math.max(...values) + 2) : 120;
  const path = buildPath(filteredPoints, minValue, maxValue);
    ...
    <section className="mt-6 rounded-md border border-foreground/10 p-5">
      ...
      <div className="mt-5 overflow-x-auto">
        <svg
          ...
          viewBox="0 0 760 260"
        >
          <line stroke="currentColor" strokeOpacity="0.12" x1="52" x2="736" y1="20" y2="20" />
          <line stroke="currentColor" strokeOpacity="0.12" x1="52" x2="736" y1="121" y2="121" />
          <line stroke="currentColor" strokeOpacity="0.12" x1="52" x2="736" y1="222" y2="222" />
          <text className="fill-current text-[11px]" opacity="0.55" x="8" y="24">
            {maxValue}kg
          </text>
          <text className="fill-current text-[11px]" opacity="0.55" x="8" y="125">
            {Math.round((minValue + maxValue) / 2)}kg
          </text>
