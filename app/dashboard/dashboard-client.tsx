...
function ExpenseBlock({
  ...
  const recordsHref = entity
    ? `/dashboard/records?tab=receipts&entity_id=${entity.id}`
    : "/dashboard/records?tab=receipts";
  const reportsHref = entity
    ? `/dashboard/reports?type=expense&entity_id=${entity.id}`
    : "/dashboard/reports?type=expense";
